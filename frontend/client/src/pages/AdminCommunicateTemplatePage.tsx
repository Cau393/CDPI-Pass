...
export default function AdminCommunicateTemplatePage({
  embeddedInHub = false,
}: {
  ...
  const { data: templateData, isFetching } = useQuery<{ body: string; subject: string }>({
    queryKey: ["/api/admin/events", selectedEvent?.id, "communicate-template"],
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/admin/events/${selectedEvent!.id}/communicate-template`,
      );
  ...
  return (
    <TooltipProvider>
      ...
      >
        {!embeddedInHub ? (
          <div>
            <p className="text-sm text-muted-foreground">Admin / Template comunicado</p>
            <h1 className="text-2xl font-bold tracking-tight">Template de comunicado</h1>
          </div>
        ) : (
          <h2 className="text-xl font-semibold tracking-tight">Template de comunicado</h2>
        )}
        ...
        {!selectedEvent ? (
          <div className="flex min-h-[320px] items-center justify-center rounded-lg border border-dashed p-8 text-center text-muted-foreground">
            Selecione um evento acima para carregar ou criar o template de comunicado.
          </div>
            ...
            <div className="min-w-0 flex-1 space-y-4">
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>
                    Texto simples (sem HTML). Variáveis:{" "}
                    {DYNAMIC_VARIABLES.map((v) => v.tag).join(", ")}.
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-0">
                  <Label htmlFor="communicate-email-subject" className="sr-only">
                    Assunto
                  </Label>
                  <Input
                    id="communicate-email-subject"
                    type="text"
                    placeholder='Ex.: Comunicado — {evento}'
                    value={subjectInput}
                    maxLength={998}
                    onChange={(e) => {
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    Deixe em branco para usar o assunto padrão do sistema.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Este envio não inclui link de resgate de cortesia. Variáveis
                    desconhecidas permanecem no texto.
                  </p>
                </CardContent>
              </Card>
            </div>
