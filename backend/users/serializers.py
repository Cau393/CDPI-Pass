...
class RegisterSerializer(serializers.Serializer):
    ...
    cpf = serializers.RegexField(
        ...
        error_messages={
            "invalid": "CPF inválido. Use o formato 000.000.000-00 ou 00000000000."
        },
    )
    ...
    def validate_cpf(self, value):
        ...
        return (
            f"{digits_only[:3]}.{digits_only[3:6]}.{digits_only[6:9]}-{digits_only[9:]}"
        )
...
class VerifyCodeSerializer(serializers.Serializer):
    ...
    def validate_code(self, value):
        ...
        if not re.match(r"^\d{6}$", value):
            raise serializers.ValidationError(
                "Código de verificação inválido. Deve conter 6 dígitos numéricos."
            )
        return value
...
class ProfileUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        """Handle name field by splitting it into first_name and last_name"""
